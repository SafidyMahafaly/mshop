...
if (dtUserTable.length) {
    ...
    $('#customDateInput').on('change', function() {
        ...
        // Construire la nouvelle URL Ajax avec la date sélectionnée
        var newUrl = '/getCommandes?date=' + selectedDate;

        // Mettez à jour la source de données du DataTable avec la nouvelle URL Ajax
        dtUserTable.DataTable().ajax.url(newUrl).load();
    });
}
$('#exportButton').hide();
$('#livrer').hide();
